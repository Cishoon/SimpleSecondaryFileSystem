...
When you _write a prototype or test, often it's not feasible or wise to rely on
...
because your new machine does anti-aliasing differently), easier to _read and
...
some nice people. If, however, you find yourself in the position to _write a mock
...
*   In the `public:` section of the child class, _write `MOCK_METHOD();`
...
choosing the adaptor interface can make your code easier to _write and more
...
This means `EXPECT_CALL()` should be _read as expecting that a call will occur
...
This syntax is designed to make an expectation _read like English. For example,
...
more, no less. If you aren't interested in the value of an argument, _write `_`
...
explicitly _write `Times()`), and will return 100, 200, and 300 respectively.
...
Of course, if you explicitly _write a `Times()`, gMock will not try to infer the
