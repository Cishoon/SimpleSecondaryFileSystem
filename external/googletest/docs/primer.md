...
*GoogleTest* helps you _write better C++ tests.

GoogleTest is a testing framework developed by the Testing Technology team with
Google's specific requirements and constraints in mind. Whether you work on
Linux, Windows, or a Mac, if you _write C++ code, GoogleTest can help you. And it
...
We'll now explain how to _write a test program, starting at the individual
...
3.  If necessary, _write a default constructor or `SetUp()` function to prepare
    the objects for each test. A common mistake is to spell `SetUp()` as
    **`Setup()`** with a small `u` - Use `override` in C++11 to make sure you
    spelled it correctly.
4.  If necessary, _write a destructor or `TearDown()` function to release any
    resources you allocated in `SetUp()` . To learn when you should use the
    constructor/destructor and when you should use `SetUp()/TearDown()`, _read
...
As an example, let's _write tests for a FIFO queue class named `Queue`, which has
...
Now we'll _write tests using `TEST_F()` and this fixture.
...
Most users should *not* need to _write their own `main` function and instead link
with `gtest_main` (as opposed to with `gtest`), which defines a suitable entry
point. See the end of this section for details. The remainder of this section
should only apply when you need to do something custom before the tests run that
cannot be expressed within the framework of fixtures and test suites.

If you _write your own `main` function, it should return the value of
`RUN_ALL_TESTS()`.
